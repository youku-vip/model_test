...
This wraps ``run/finetune_isaac05.py`` instead of duplicating its dataset/
patching pipeline. The base script still owns validation, processor/model
patching, checkpointing and LeRobot arguments; this wrapper only rewrites the
Accelerate launch backend to DeepSpeed ZeRO-3.

Why ZeRO-3, not ZeRO-2?
-----------------------
Isaac-0.5 is a 36B-class model. In BF16, parameters alone are ~72 GB, before
runtime buffers and activations. ZeRO-2 partitions optimizer/gradient state
but keeps parameters replicated, which is too tight for an A100-80GB. ZeRO-3
...
# Running a file from /code/run puts /code/run first on sys.path. Add repo root
# explicitly so ``from run import finetune_isaac05`` works reliably.
...
def _rewrite_launch_command(cmd: list[str], config: Path) -> list[str]:
    ...
    try:
        ...
        raise RuntimeError(
            "finetune_isaac05.py 生成的 accelerate launch 命令结构发生变化"
        ) from exc
    ...
    if target_idx is None:
        raise RuntimeError(
            "找不到 lerobot.scripts.lerobot_train，无法改写 DeepSpeed 启动命令"
        )

    accelerator_args = [
        arg
        for arg in cmd[launch_idx + 2 : target_idx]
    ...
    # When a DeepSpeed config file is supplied, precision and ZeRO settings
    # are intentionally owned by that file instead of duplicated on the CLI.
    accelerator_args += [
        "--use_deepspeed",
...
def _patch_launch_training() -> None:
    ...
    def launch_training(cmd: list[str], **kwargs) -> None:  # type: ignore[no-untyped-def]
        config = Path(
            os.environ.get("ISAAC_DEEPSPEED_CONFIG", str(DEFAULT_CONFIG))
        ).resolve()
...
def main() -> None:
    ...
    # Preserve the original CLI exactly: dataset checks, RTC, Flow MC, action
    # LR, save-fp32, resume, W&B, and all existing arguments still work.
