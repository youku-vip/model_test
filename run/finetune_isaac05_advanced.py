...
def _add_train_scope_args(cmd: list[str], scope: str, lora_r: int, lora_alpha: int, lora_dropout: float) -> list[str]:
    ...
    head, tail = cmd[:target_idx], cmd[target_idx:]
...
def main() -> None:
    ...
    # fsdp1 is the legacy stable FSDP path from finetune_isaac05.py. Explicitly
    # request it before handing argv back to the base parser.
    if known.backend == "fsdp1" and "--fsdp" not in remaining:
        remaining = [*remaining, "--fsdp"]
